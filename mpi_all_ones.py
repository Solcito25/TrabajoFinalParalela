...
if rank == 0:
    n = comm.Get_size() - 1  # Total Number of workers
    kA = int(sys.argv[1])
    kB = int(sys.argv[2])
    k = kA * kB
    ...
    for i in range(0, n):
        ...
        P[i] = np.hstack(
            (np.vstack(np.kron(M_A[i][j, :], M_B[i][mm, :]) for mm in range(0, lb1))).T for j in range(0, la1))
                ...
                for rr in range(0, k):
                    ...
                    if j >= (i - k) * (r_ind - 1) and j <= (i - k) * (r_ind - 1) + qA - 1:
                        if jj >= (i - k) * (c_ind - 1) and jj <= (i - k) * (c_ind - 1) + qB - 1:
                            r1 = j - (i - k) * (r_ind - 1)
                            c1 = jj - (i - k) * (c_ind - 1)
                            sumA = sumA + Wab[rr, r1 * qB + c1] * R_A[int(r_ind - 1), i - k] * R_B[
                                int(c_ind) - 1, i - k]
